...
export function VideoPlayer(){
    ...
    useEffect(() => {
        if(login && token){
            (async () => {
                try{  
                    const api = `https://Video-Library-Backend.sauravkumar007.repl.co/userdata/history/${videoId}`
                            ...
                            <div>
                                {
                                    ...
                                    :
                                    <span 
                                        onClick={ login ? () => addToLike(video, _id) : () => dispatch({type: "TOGGLE_LOGIN_MODAL"}) }
                                    > 
                                        <i className="fas fa-thumbs-up"></i> 
                                    </span>
                                }
                                {
                                    ...
                                    :
                                    <span 
                                        onClick={ login ? () => addToWatchLater(video) : () => dispatch({type: "TOGGLE_LOGIN_MODAL"}) }
                                    >
                                        <i className="fas fa-clock"></i>
                                    </span>
                                }
